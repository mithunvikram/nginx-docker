...
export class OrganizationComponent implements OnInit {
  ...
  displayedColumns = ['Id', 'Organization', 'PolypmCustomerCode', 'Actions'];
  ...
  constructor(private spinnerservice: Ng4LoadingSpinnerService, private organizationService: OrganizationService,
    private router: Router, private dailog: MatDialog) { }
  ...
  getAllOrganization() {
    ...
    this.organizationService.getAllOrganization().subscribe(
      data => {
        console.log('getAllOrganization in main ts ---- ', data);
        if (data === 'There is no Organisation') {
          ...
          if (organizationDetails !== null) {
            if (organizationDetails.PolypmCustomerCode !== null && organizationDetails.PolypmCustomerCode !== 'STAHLS') {
              this.listOfOrganization.forEach(element => {
                if (element.PolypmCustomerCode === organizationDetails.PolypmCustomerCode) {
          ...
          this.dataSource.sortingDataAccessor = (item, property) => {
            console.log('etnering into sortingDataAccessor ', item, property);
            switch (property) {
  ...
  Popup() {
    ...
    this.dialogConfig.data = 'No Data in Organization';
  ...
  Opendialog(row) {
    ...
    const dialogRef = this.dailog.open(DeleteDialogComponent, this.dialogConfig);
